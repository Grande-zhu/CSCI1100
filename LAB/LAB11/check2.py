...
class BallDraw(object):
    ...
    def draw_ball(self):
        ...
        self.canvas.create_oval(Ball.bounding_box(self), fill=self.ball_color)
    ...
    def animate(self):
        ...
        while(Ball.some_inside(self,self.maxx,self.maxy)) and (not self.isstopped):
            # Move the ball
            self.draw_ball()
            if(Ball.check_and_reverse(self,self.maxx,self.maxy))=="x":
                self.ball_dx=0-self.ball_dx
            elif(Ball.check_and_reverse(self,self.maxx,self.maxy))=="y":
                self.ball_dy=0-self.ball_dy
                
            self.ball_x += self.ball_dx
            self.ball_y += self.ball_dy
            #print(self.maxx,self.maxy,Ball.position(self))
